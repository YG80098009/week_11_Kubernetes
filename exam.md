...
[https://octopus.com/devops/kubernetes-deployments/kubernetes-yaml-types/](https://octopus.com/devops/kubernetes-deployments/kubernetes-yaml-types/)
...
1. **Dockerfile** \- קובץ ה-Docker שכתבתם   
2. **pod.yaml** \- קובץ ה-Pod ל-Kubernetes   
3. **service.yaml** \- קובץ ה-Service ל-Kubernetes   
4. **commands.txt** \- קובץ טקסט עם כל פקודות ה-CLI שהרצתם (מהשלב הראשון ועד האחרון)  
5. **screenshot.png** \- צילום מסך של האפליקציה פועלת בדפדפן (כולל ה-URL בשורת הכתובת)
...
## **שלב 1: הכנת הסביבה** **1.1 הורדת הקוד מ-GitHub** 
...
[https://github.com/yosefanaliza/streamlit\_minikube\_app](https://github.com/yosefanaliza/streamlit_minikube_app)
...
## **שלב 2: בניית Docker Image (ניקוד 27%)** **2.1 כתיבת Dockerfile עבור האפליקציה**
...
לא לשכוח לשמור את פקודות ה CLI הרלוונטיות לגישה לדוקר האב \- תיוג, דחיפה ו-ולידציה ב-commands.txt.

## **שלב 3: פריסה ל-Kubernetes (ניקוד 53%)**  **3.1 כתיבת קובץ pod.yaml**
...
## **שלב 4: הרצת האפליקציה ב-Minikube (ניקוד 15%)** **4.1 יישום  (apply) הקבצים**
...
## **שלב 5: בדיקות נוספות (ניקוד 5%)** **5.1 צפייה בלוגים**
...
* **אל תשכחו את `--server.address=0.0.0.0`** \- בלעדיו Streamlit לא יהיה נגיש מחוץ ל-container (המושג נקרא bind address והוא מציין מאיזה IP ניתן לגשת לשרת ו 0.0.0.0 מאפשר גישה מכל IP)  
  פקודת ההרצה עם הקונפיגורציה הנ"ל:  
  streamlit run your\_app.py \--server.port 8080 \--server.address=0.0.0.0
